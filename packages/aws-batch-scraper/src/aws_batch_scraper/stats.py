...
def print_failures(s3: S3Client, config: WorkerConfig, run_id: str) -> None:
    ...
    for page in paginator.paginate(Bucket=config.s3_bucket, Prefix=prefix):
        for obj in page.get("Contents", []):
            key_item_id = obj["Key"].removeprefix(prefix).removesuffix(".json")
            body = s3.get_object(Bucket=config.s3_bucket, Key=obj["Key"])["Body"].read()
            try:
                rec = json.loads(body)
                if not rec.get("item_id"):
                    rec["item_id"] = key_item_id
                failures.append(rec)
            except Exception:
                failures.append({"item_id": key_item_id, "error": "could not parse JSON"})
    ...
    for f in sorted(failures, key=lambda x: x.get("item_id", "")):
        item_id = f.get("item_id", "?")
        classification = f.get("classification", "?")
        subreason = f.get("subreason", "")
        logger.info(f"  {item_id}  {classification}  {subreason}")
...
def print_run_stats(s3: S3Client, config: WorkerConfig, run_id: str) -> None:
    ...
    total_items = sum(w.get("items_processed", 0) for w in worker_stats)
    ...
    ipm_values = [w["items_per_minute"] for w in worker_stats if w.get("items_per_minute")]
    ...
    if ipm_values:
        logger.info(f"Per-worker throughput: {[round(v, 1) for v in ipm_values]} items/min")
        logger.info(f"Combined throughput: {sum(ipm_values):.1f} items/min")
    ...
    try:
        ...
        run_items = [
            json.loads(line)["item_id"] for line in body.decode().splitlines() if line.strip()
        ]
