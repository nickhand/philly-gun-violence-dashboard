...
class UJSPortalScraper:
    ...
    Implements the ``aws_batch_scraper.types.Scraper`` Protocol via structural
    ...
    def _before_retry(self, retry_state: tenacity.RetryCallState) -> None:
        ...
        # After 1 retry on SOFT_BLOCKED, give up in-task and return is_soft_blocked=True
    ...
    def failure_artifacts(self, item: WorkItem) -> list[FailureArtifact]:
        """Return screenshot and HTML diagnostics for a permanent failure."""
        ...
        try:
            artifacts.append(
                FailureArtifact(
                    suffix="png",
                    body=self._page.screenshot(full_page=True),
        ...
        try:
            artifacts.append(
                FailureArtifact(
                    suffix="html",
                    body=self._page.content().encode(),
    ...
    def __call__(self, item: WorkItem) -> ScrapeResult:
        """Scrape portal results for a single item with retries.

        SOFT_BLOCKED outcomes are caught internally and returned as a
        ``ScrapeResult`` with ``is_soft_blocked=True`` rather than propagated
        to the worker as an exception.
        """
        input_value = item.item_id
        normalized = self._normalize_input(input_value)
        if normalized is None:
            return ScrapeResult(
        ...
        try:
            ...
            # Internally soft-blocked — signal the worker via flag, not exception
            return ScrapeResult(
                status=ScrapeStatus.FAILED,
                classification="SOFT_BLOCKED",
                subreason="SOFT_BLOCKED after retry budget — deferred to SQS",
        ...
        is_network_error = final_result.classification == Classification.NETWORK_OR_SERVER_ERROR
        ...
        return ScrapeResult(
            status=status,
            data={"results": [r.model_dump() for r in portal_results]} if portal_results else None,
