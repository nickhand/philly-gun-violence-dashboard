  ...
  <div class="header-message-wrapper w-100">
    ...
    <div>
      ...
      <div style="position: relative">
        ...
        <div v-else class="header-submessage pl-5 pr-5">
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      /**
       * Error loading homicide totals
       */
      homicideLoadError: null as string | null,
    };
  },
   ...
   */
  async created() {
    await this.reloadHomicideTotals();
  },
  computed: {
    /**
     * Whether homicide stats are available
     */
    hasHomicideData(): boolean {
      if (!this.homicideData) return false;
      if (this.selectedYear === null) return this.getTotalHomicideCount() !== null;
      if (this.selectedYear) return this.getHomicideTotal(this.selectedYear) !== null;
      return false;
    },
  ...
  methods: {
    /**
    * Load or reload homicide totals from GitHub
    */
    async reloadHomicideTotals() {
      try {
        this.homicideData = await githubFetch("homicide_totals.json");
        this.homicideLoadError = null;
      } catch (error) {
        console.error("Failed to fetch homicide totals", error);
        this.homicideLoadError =
          "Homicide totals are currently unavailable. Please try again later.";
      }
    },
