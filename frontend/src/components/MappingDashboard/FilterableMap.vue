...
export default defineComponent({
  ...
  data() {
    ...
    let data = {
      ...
      /** Debounce handle for map hash updates */
      hashUpdateTimeout: null as number | null,
    };
  ...
  watch: {
    ...
    mapHash(newVal, prevVal) {
      ...
      this.scheduleHashUpdate(newVal);
    },
  },

  methods: {
    /**
     * Debounce map hash updates before pushing to the router
     */
    scheduleHashUpdate(newHash: string) {
      if (this.hashUpdateTimeout) {
        window.clearTimeout(this.hashUpdateTimeout);
      }

      this.hashUpdateTimeout = window.setTimeout(() => {
