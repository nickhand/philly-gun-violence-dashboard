...
import { SourceConfig, DataFeatureCollection } from "@/types/Sources";
...
export default defineComponent({
  ...
  methods: {
    ...
    /**
     * Fetch source data if it is not already loaded (used for downloads)
     */
    async ensureSourceDataForDownload(
      source: SourceConfig
    ): Promise<DataFeatureCollection | undefined> {
      ...
      // Batch or single query
      if ("batchSize" in source) {
        const where_key = source.whereColumn;
        if (where_key === undefined) return undefined;

        const values = Array.from(
          new Set(
            this.filteredData
              ?.map((d: GenericFeature) => d.properties[where_key])
              .filter((d: any) => d)
          )
        );

        const data = await queryFeatureServerBatch({
          url: source.url,
          where_key,
          where_value: values,
        ...
        const data = await queryFeatureServer({
          url: source.url,
          outFields: source.outFields,
          geometryPrecision: source.geometryPrecision,
          where: source.where,
     ...
     */
    async handleAggregatedDataDownload(
      ...
      // Handle GeoJson
        ...
        // Get the source and its data (fetch if needed)
        let source = this.getSource(layer);
        let geodata = await this.ensureSourceDataForDownload(source);
        if (!geodata) {
          throw new Error("No data available for aggregated download");
        }

        // Loop over all features
        geodata.features.map((feature) => {
          // Clone so we do not mutate the live map data
          let clonedFeature = JSON.parse(
            JSON.stringify(feature)
          ) as GenericFeature;

          // Get the name and count
          let name = clonedFeature.properties[layer.geoid];
          let count = aggCountMap.get(name) || 0;

          // Overwrite the properties
          clonedFeature.properties = {};
          clonedFeature.properties[layer.name] = name;
          clonedFeature.properties["count"] = count;

          // Add to our collection
          collection.features.push(clonedFeature as GenericFeature);
        });
    ...
    handleDownload(params: DataDownloadParams) {
      ...
      if (params.aggLayer !== null) {
        ...
        // Work on a deep copy so we never mutate the map/chart state
        let downloadFeatures = JSON.parse(JSON.stringify(data));

        let toFormat = Object.keys(formatters);
        if (exclude.length || toFormat.length > 0) {
          downloadFeatures.map((d: GenericFeature) => {
        ...
        if (params.outputType == "CSV") {
          content = jsonToCSV(downloadFeatures);
          fileName = "data.csv";
          contentType = "text/plain";
          // GeoJSON
        } else {
          content = jsonToGeoJson(downloadFeatures);
