...
def read_processed_geojson(name: str, *, s3: S3Client | None = None) -> gpd.GeoDataFrame:
    ...
    return read_geojson_gdf(_client(s3), get_s3_settings().s3_bucket, key)


def read_processed_geojson_json(name: str, *, s3: S3Client | None = None) -> Any:
    """Read a processed GeoJSON dataset from S3 as JSON."""
    key = get_processed_key(name)
    return read_json(_client(s3), get_s3_settings().s3_bucket, key)
...
def read_processed_csv(
    ...
    **read_csv_kwargs: Any,
    ...
    key = get_processed_key(name)
    return read_csv_df(_client(s3), get_s3_settings().s3_bucket, key, **read_csv_kwargs)
...
def write_processed_geojson(
    ...
    *,
    ...
    key = get_processed_key(name)
    write_geojson_gdf(_client(s3), get_s3_settings().s3_bucket, key, gdf)
...
def write_processed_json(name: str, data: Any, *, s3: S3Client | None = None) -> None:
    ...
    key = get_processed_key(name)
    write_json(_client(s3), get_s3_settings().s3_bucket, key, data)
